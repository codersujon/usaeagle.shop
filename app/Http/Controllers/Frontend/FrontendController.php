...
class FrontendController extends Controller
{
    ...
    public function index(){
        $hotdeal_top = Product::where(['status' => 1, 'topsale' => 1])
                ...
                ->limit(12)
                ->get();
        return view('frontEnd.layouts.landingMaster', compact('hotdeal_top'));
    }
    ...
    public function campaign($slug)
    {
        ...
        $productsizes = Productsize::where('product_id', $campaign_data->product_id)
            ->with('size')
            ->get();
        $product_colors = Productcolor::where('product_id', $campaign_data->product_id)
            ->with('color')
            ->get();
        Session::put('shipping', $select_charge->amount);
        return view('frontEnd.layouts.pages.campaign.campaign', compact('campaign_data', 'product', 'shippingcharge', 'productsizes', 'product_colors'));
    }
